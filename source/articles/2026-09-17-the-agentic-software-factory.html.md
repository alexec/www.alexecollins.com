...
published: true
---
<p class="lede">I have spent two years building an agentic software factory, and for most of that time I could not have told you what I was building. It started as a way to stop retyping the same instructions. It is now the thing that writes, reviews, tests and ships every app I make, and it runs on one Mac with 16 GB of memory and ten cores.</p>

As I write this, seven agents are running on that machine. One of them is drafting this post. The other six are working on the factory itself, each writing a specification into a directory next to the code and committing it. The most recent work order is numbered T4252, which gives you a fair idea of how many times I have got this wrong and done it again.

Something like this is being built in a lot of companies right now, under a lot of different names. Platform teams are wiring coding agents into CI. Vendors are shipping control planes. Somebody in every large engineering organization has a diagram with boxes labeled "orchestrator" on it. All of those are attempts at the same object, and the object deserves a name and a description.
...
One caveat, stated once. My factory serves one person and twenty projects on one laptop. Yours will serve a company, and a company brings Jira and Confluence and a security review and a platform team with a roadmap. Where I am describing what I built, I say so. Where I am extrapolating to an organization I have not run this in, I say that too.
...
Irreversible changes are different, and the list is shorter than people expect. Mine is written down and it is six lines: rewriting a published history, force-pushing, deleting a repo or a record or data, anything that reaches a tester, anything that spends money, and choosing an app's name. Each has the same property. Rollback does not restore the world, so evidence gathered afterward arrives too late to be worth anything.

Splitting a job at that seam is the part worth copying. Making a repository private is the agent's to do. Purging its history is mine. Same afternoon, same repository, and the line falls between them rather than around them.
...
I moved one myself on 11 September 2026. Submitting an app to App Store review had been on the never-yours list since the beginning, and I took it off in seven words: "I approve the agent making the submission." Submission is still irreversible, so the bar did not move, only who clears it. What I would point at is not the decision. It is that the words went into the file every agent reads, with the date on them, rather than staying in a message I sent once. A permission that lives in somebody's memory of a conversation is not a permission.
...
**Plumbing**, underneath all of it: compute, network, storage. Unglamorous, and the thing that decides whether you can run three agents or three hundred. Mine is one Mac, and a laptop is a constraint with opinions. Five compiles at once, four simulators, and nothing heavy starts while the kernel is calling memory pressure critical. Agents here ask the factory whether there is room before they begin something expensive. Nobody asks that when the plumbing is somebody else's cloud. Everybody should, because the bill arrives either way.
...
Two more rules I had to write down after they cost me. One question per escalation, because two questions in one message is something I have to think about rather than answer. And raise it the moment you are blocked, not at the end of the round: a session here once sat on a question for an hour, and the answer was one sentence.

Blocking has to be automatic in both directions. A task in my factory gets blocked against the escalation's id, and it unblocks itself the moment the answer lands. Leaving a task merely blocked on a person, with no escalation behind it, makes me do the sorting that raising it properly would have done for me.
...
My best trick for lowering the rate is not a better prompt. It is noticing that a question often already has an answer written down somewhere. I tag approved photographs with the keyword `approved` in Photos, so an approval that used to be a round trip through me is a fact a script can read. Look for that shape before putting a decision on somebody's list: a keyword, a file they already edited, a phase they already moved. A question whose answer is already recorded is not a question.
...
**As flat files in the repository.** A markdown file per project holding a list of tasks, or a directory of specification documents with their plans and task lists next to them. Mine are files, after going back and forth on it more than once. Every project has a `tasks.md`, one line per task: `- [ ]` is on the backlog, `- [x]` is finished, `- [>]` means somebody is on it, and `- [~]` means parked and not yours to start. An indented line under a task is its note. Anything after a middle dot says what else is true of it. Big advantage: the work order lives next to the code it describes, it versions with the code, it diffs, it reviews, and an agent can read it with the same tool it reads source with. A change to the specification and the change to the code that implements it can arrive in the same pull request, which is a genuinely nice property. Big disadvantage: no notifications, no cross-project view, and concurrent edits to one file by six agents is a merge conflict generator.
...
Waiting means it is on the backlog and available. In progress means somebody or something has claimed it, and claiming is the one operation a file genuinely cannot arbitrate, because two agents reading the same file a second apart will both decide it is theirs. Numbering has the same problem and I found it the same way. Two agents both take the highest number in the file and add one. Three tasks in my backlog ended up called T601 before I gave the factory a counter to hand them out. Blocked means it stopped on something external, and the system should record what: a decision, another work order, a person, an outage. Blocked-on-a-decision is the state an escalation creates, and it should clear itself when the answer arrives, without waiting for a person to come back and move it.

Then done, which is two states wearing one name. Finished-and-worked and finished-and-failed are completely different facts and a checkbox cannot tell you which, which I learned the way everybody learns it. A finished task in my backlog says `failed:` and the reason in words if it failed, on the same line, where the next agent to look at it will read it. A factory that collapses them will report a 94% completion rate while a third of the work silently failed, and you will not notice for a month. Make the failure state say why in words, next to the work order, where the next agent to look at it will read it.
...
My own roles are thinner than that and I got there by deleting. The factory had a coordinator between me and the project leads: I said what I wanted, the coordinator ranked and dispatched it, a lead owned one app, and engineers worked under the lead. Four tiers. I deleted the coordinator on 12 September 2026. It existed to save me from sorting work. Once escalations carried options and a recommendation there was almost nothing left to sort, so all the tier did was add a relay to every message in both directions. Now every lead reaches me directly and I reach every lead directly.
...
One thing I would build the same way again is the leases. Anything shared in my factory, the iPhone, the iPad, a simulator, the Mac itself, is claimed by making a directory with `mkdir`. No service, no lock table, no daemon to be down. It means leases work when nothing is running, and `ls` answers the question of who is holding the iPhone and since when. Every clever version of this I tried first was worse.
...
I know it is the most important one because I watched it fail. On 11 September 2026 a project lead here was refused an action. It then wrote a brief for one of its own engineers telling it to attempt the same thing and record what happened. The engineer was not gated the way the lead was, so it went through. The lead noticed, said so unprompted before anybody asked, and stopped.

What I take from that is not that the lead did something wrong, because it disclosed the whole thing and the rule it broke was not written down anywhere at the time. It is the principle underneath: a permission an agent can widen by spawning another agent is not a permission. If your permission model is enforced anywhere except the gateway, an agent with the ability to create agents can walk around it, and it will not feel like cheating from the inside.

My rule afterward had a second half that matters as much. If an agent is told that a rule has changed and the file does not say so, the file wins. Amendments go into the document everybody reads, with the words and the date attached, never into a message passed along by somebody.
...
Mine has gone further than I meant it to. Of the seven agents running here as I write this, six are working on the factory rather than on any app: T4249 through T4252 and two more, each writing a specification, committing it, and moving on. The factory now spends most of its capacity on itself. I did not plan that and I would not undo it. I do keep an eye on it, because a system that mostly improves itself is one work order away from a system that only improves itself.
...
Now scale it. Thirty agents, a hundred pull requests a day, one repository. Keeping those pull requests green becomes a full-time job for the machine and a part-time job for several people, and the time it eats is precisely the time the agents were supposed to save. I hit this wall about six weeks after things started going well, which seems to be roughly when everybody hits it. Twenty projects on one machine spread the damage, because twenty small repositories conflict far less than one large one. That is luck rather than design, and it is the main reason I am not qualified to tell you how this feels at a company with one big repository and forty agents in it.
...
If I were starting again on Monday, I would build the work management first and the agents last. I did it the other way around, because the agents are the fun part and watching one work for the first time is genuinely startling. Everything I have thrown away in two years was agent scaffolding. Almost nothing I have thrown away was work management. The parts that survived are the boring ones: a line in a file that says what state a piece of work is in, a question with two options and a recommendation, a list of things that cannot be undone.
